import type { SlidingPuzzleState } from './types.js';
...
export function generateSlidingPuzzleInitialState(input: {
  ...
  let state: SlidingPuzzleState = { size, board };

  const rand = mulberry32(xmur3(seed)());
  let lastMovedTile: number | null = null;
  ...
  for (let i = 0; i < scrambleMoves; i++) {
    ...
    if (row > 0) adjacentTiles.push(state.board[blankIdx - size]); // up
    if (row < size - 1) adjacentTiles.push(state.board[blankIdx + size]); // down
    if (col > 0) adjacentTiles.push(state.board[blankIdx - 1]); // left
    if (col < size - 1) adjacentTiles.push(state.board[blankIdx + 1]); // right
  ...
  if (state.board.every((v, i) => (i < size * size - 1 ? v === i + 1 : v === 0))) {
    for (let i = 0; i < 10; i++) {
      ...
      const col = blankIdx % size;
      const adjacentTiles: number[] = [];
      if (row > 0) adjacentTiles.push(state.board[blankIdx - size]);
      if (row < size - 1) adjacentTiles.push(state.board[blankIdx + size]);
      if (col > 0) adjacentTiles.push(state.board[blankIdx - 1]);
      if (col < size - 1) adjacentTiles.push(state.board[blankIdx + 1]);
