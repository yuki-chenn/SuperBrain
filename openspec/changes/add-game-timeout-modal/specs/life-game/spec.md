...
The system SHALL render a dedicated timeout modal on the life-game play page whenever the attempt's client status reaches `timeout`. The modal SHALL state the time limit was reached and the attempt is not counted toward the leaderboard, and SHALL offer Restart and Back-to-games actions.

#### Scenario: Timeout reached during play
- **WHEN** the player has an in-flight life-game attempt and the elapsed playing time meets or exceeds the difficulty's `maxDurationMs`
...
#### Scenario: Server-detected timeout on region submit
- **WHEN** a region submission returns a timeout failure (the server has marked the attempt `INVALID/TIMEOUT`)
...
The system SHALL display the remaining time (computed as `max(0, maxDurationMs - elapsedMs)`) in `mm:ss` format inside the life-game play page HUD while the attempt is in the `playing` state, and SHALL apply a visual warning style when the remaining time is less than or equal to 30 seconds.
...
The system SHALL preserve `packages/shared/src/games/life-game/config.ts` as the only place to change the difficulty `maxDurationMs`, and SHALL document this fact at the top of that file.

#### Scenario: Config edit updates both API and Web without code changes elsewhere
- **WHEN** a developer edits `maxDurationMs` for a difficulty in `packages/shared/src/games/life-game/config.ts` and rebuilds shared
- **THEN** the API's `getLifeGameMaxDurationMs(key)` returns the new value
