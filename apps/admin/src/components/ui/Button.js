...
export const Button = forwardRef(({ variant = 'primary', size = 'md', className = '', ...props }, ref) => {
    return (_jsx("button", { ref: ref, className: `inline-flex items-center justify-center font-medium border transition-[var(--sb-transition)] rounded-[var(--sb-radius-button)] hover:-translate-y-[1px] cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0 ${variantStyles[variant]} ${sizeStyles[size]} ${className}`, ...props }));
});
